...
namespace Api.Controllers
{
    public class ContratoController : BaseController
    {
        ...
        {
            var entity = await _unitOfWork.Contratos.GetAllAsync();
            return _mapper.Map<List<ContratoDto>>(entity);
        }
        [HttpPost]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<Contrato>> Post(ContratoDto entityDto)
        {
            var entity = _mapper.Map<Contrato>(entityDto);
        /*
        ...
        */
        ...
        {
            var entity = await _unitOfWork.Contratos.GetByIdAsync(id);
            if (entity == null)
            {
                return NotFound();
            }
            return _mapper.Map<ContratoDto>(entity);
        }
        [HttpPut("{id}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<ContratoDto>> Put(int id,ContratoDto entityDto)
        {
            var entity = _mapper.Map<Contrato>(entityDto);
            if (entity.Id == 0)
            {
        ...
        */
            entityDto.Id = entity.Id;
            _unitOfWork.Contratos.Update(entity);
            await _unitOfWork.SaveAsync();
            return entityDto;
        }
        [HttpDelete("{id}")]
        ...
        public async Task<ActionResult> Delete(int id)
        {
            ...
            _unitOfWork.Contratos.Remove(entity);
            await _unitOfWork.SaveAsync();
            return NoContent();
        }
    }
